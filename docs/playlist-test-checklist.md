...
22. Use **Up**/**Down**, drag/drop on selected channels within one group, and drag a channel onto a custom group, then confirm **Custom order** and custom group assignment persist.
...
28. Import XMLTV from `.xml`, `.gz`, or `.zip` when available, then verify the selected channel **EPG guide** and EPG search show matching programs.
29. Open the VOD library poster grid, page through entries, play a VOD/series item, and confirm resume progress updates when playback position is available.
30. Attempt playback and confirm **Stream Health** updates with success/failure/buffering counts and fallback scores.
31. Confirm normal UI does not show full tokenized stream URLs.
