...
5. If playback does not start, confirm the app shows a clear timeout/failure message.
6. Click **Import File** and select a user-provided `.m3u` or `.m3u8`.
7. Confirm the import summary shows channel, warning, duplicate, and error counts.
8. Search/filter by group/category.
9. Mark a channel as favorite, close the app, reopen, and re-import the same playlist to confirm the favorite reappears.
10. Confirm normal UI does not show full tokenized stream URLs.
